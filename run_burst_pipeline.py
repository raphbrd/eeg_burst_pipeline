...
Notes
-----
- The input data of this pipeline should be continuous resting-state EEG recordings, in the format .fif. The files will be loaded
through the `mne.io.read_raw_fif` function and thus should be compatible with the mne.io.Raw API. 
- This pipeline assumes all data were preprocessed and ready to analysis. 
...
# /!\ Replace the following part by your own way to get :
...
sfreq: float = 1000  # sampling frequency
...
# computing cycles characteristics on all sensors for every window. those data can be filtered
# dir_prefix should be identical to the whole block computation, as this run is using the same cycles decomposition
# and characteristics computed at step 1
